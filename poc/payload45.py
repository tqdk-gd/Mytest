...
fake = Faker()
# 创建命令行参数解析器
parser = argparse.ArgumentParser(description='发送带有指定URL的GET请求。存在问题的路径为/admin/add_postlogin.php',formatter_class=argparse.RawTextHelpFormatter)
...
if args.url:
    if args.url.endswith('/'):
        url = args.url[:-1] + "/admin/add_postlogin.php"  # 移除末尾的斜杠
    else:
        url = args.url + "/admin/add_postlogin.php"
else:
