...
fake = Faker()
# 创建命令行参数解析器
parser = argparse.ArgumentParser(description='发送带有指定URL的POST请求。存在问题的路径为/third/wechat/wechatLoginHelper.do，参数是uid和method',formatter_class=argparse.RawTextHelpFormatter)
...
if args.url:
    if args.url.endswith('/'):
        url = args.url[:-1] + "/third/wechat/wechatLoginHelper.do"  # 移除末尾的斜杠
    else:
        url = args.url + "/third/wechat/wechatLoginHelper.do"
else:
